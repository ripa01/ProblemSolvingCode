...
void save()
{
	...
	printf("Phone Number\t\tUser Name\t\tGender Name\t\tDistric\t\tAge\t\tAmount\n");

	while(fwrite(&p,sizeof(struct clientrecord),1,fp)==1)
	{
		printf("\n%-10s\t\t%-20s\t\t%-20s\t\t%-20s\t\t%d\t\tTK %.2f /- \n",p.pn,p.name,p.gender,p.distric,p.age,p.amount);
	}
	printf("\n");
